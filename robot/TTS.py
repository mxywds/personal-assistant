...
from .sdk import TencentSpeech, AliSpeech, XunfeiSpeech, atc, VITSClient, ChatTTSSpeech
...
class ChatTTS(AbstractTTS):
    """
    ChatTTS 语音合成
    需要自行搭建ChatTTS：https://github.com/2noise/ChatTTS
    """
    SLUG = "chat-tts"

    def __init__(self, seed, temperature, top_P, top_K, use_decoder):
        ChatTTSSpeech.load()
        super(self.__class__, self).__init__()
        self.seed, self.temperature, self.top_P, self.top_K, self.use_decoder = \
            (seed, temperature, top_P, top_K, use_decoder)

    @classmethod
    def get_config(cls):
        return config.get("chat-tts", {})

    def get_speech(self, phrase):
        tmp_file = ChatTTSSpeech.tts(phrase, self.seed, self.temperature, self.top_P, self.top_K, self.use_decoder)
        logger.info(f"{self.SLUG} 语音合成成功，合成路径：{tmp_file}")
        return tmp_file
